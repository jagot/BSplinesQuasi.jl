...
Generate the Gauß–Legendre quadrature roots `x` and associated weights
`w` necessary to compute matrix elements of a polynomial operator of
order `k′`, with respect to the B-spline basis generated by the knot
set `t`.
